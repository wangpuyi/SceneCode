...
class BlenderMCPServer:
                ...
                try:
                    ...
                    while b'\n' in buffer:
                        ...
                        try:
                            ...
                            print(f"Invalid JSON from client (len={len(line)}), skip line")
                            continue

                        # Execute command in Blender's main thread（用 cmd 绑定当前 command，避免 timer 延后执行时拿到下一轮的值）
    ...
    def _execute_command_internal(self, command):
        ...
        # 兼容两种格式：code 在 params 中 或 在 command 顶层（部分客户端会这样发）
        if cmd_type == "execute_code" and "code" not in params and "code" in command:
            params["code"] = command["code"]
        ...
        if handler:
            try:
                if cmd_type == "execute_code":
                    ...
                    print(f"Executing handler for {cmd_type} (code length={code_len})")
                else:
    ...
    def get_scene_bounds(self):
        """
        计算当前场景中所有网格物体的整体包围盒（世界坐标 AABB）。
        用于部件特写时让相机对准部件中心并拉近。
        Returns:
            dict: min, max, center, size (均为 [x,y,z])；若无网格则返回 None
        """
        try:
            import mathutils
            min_corner = None
            ...
            for obj in bpy.context.scene.objects:
                ...
                if min_corner is None:
                    ...
                    for i in range(3):
                        min_corner[i] = min(min_corner[i], lo[i])
                        max_corner[i] = max(max_corner[i], hi[i])
            if min_corner is None:
                return None
            ...
            print(f"Error in get_scene_bounds: {str(e)}")
            traceback.print_exc()
            return None

    def list_scene_object_names(self):
        """
        返回场景中所有对象的名称与类型，用于特写时筛选“当前部件”对象。
        Returns:
            list of {"name": str, "type": str}，不含数量限制
        """
        try:
            return [
            ...
            print(f"Error in list_scene_object_names: {str(e)}")
            return []

    def hide_objects_except(self, keep_visible_names):
        """
        渲染时只保留指定名称的对象可见，其余对象设为 hide_render=True。
        相机与灯光始终不隐藏，以便正常渲染。
        Args:
            keep_visible_names: 需要保持可见的对象名称列表（通常为当前部件对应的 mesh 名称）
        """
        if not isinstance(keep_visible_names, (list, tuple)):
            keep_visible_names = list(keep_visible_names) if keep_visible_names else []
        keep_set = set(keep_visible_names)
        for obj in bpy.context.scene.objects:
    ...
    def show_all_objects(self):
        """将所有对象的 hide_render 设为 False，恢复全部参与渲染。"""
        for obj in bpy.context.scene.objects:
    ...
    def setup_lighting(self, light_type="MULTI_POINT", intensity=1.0):
        """
        设置场景光照
        
        Args:
            light_type: 光照类型 ("THREE_POINT", "STUDIO", "HDRI", "SIMPLE", "MULTI_POINT")
            intensity: 光照强度倍数
        ...
        try:
            ...
            if light_type == "THREE_POINT":
                # 三点光照：主光、补光、轮廓光
                light_configs = [
                    {"name": "Key_Light", "type": "SUN", "position": (4, -4, 6), "energy": 3.0 * intensity, "color": (1.0, 0.98, 0.95)},
                    {"name": "Fill_Light", "type": "AREA", "position": (-4, -2, 3), "energy": 1.5 * intensity, "color": (0.9, 0.95, 1.0)},
                    {"name": "Rim_Light", "type": "SPOT", "position": (-2, 4, 4), "energy": 2.0 * intensity, "color": (1.0, 1.0, 1.0)},
                ]
            elif light_type == "STUDIO":
                # 工作室光照
                light_configs = [
                    {"name": "Top_Light", "type": "AREA", "position": (0, 0, 5), "energy": 5.0 * intensity, "size": 3.0},
                    {"name": "Front_Light", "type": "AREA", "position": (0, -4, 2), "energy": 2.0 * intensity, "size": 2.0},
                ]
            elif light_type == "SIMPLE":
                # 简单光照
                light_configs = [
                    {"name": "Sun", "type": "SUN", "position": (4, -4, 6), "energy": 4.0 * intensity},
                ]
            elif light_type == "MULTI_POINT":
                # 多点光：保轮廓 + 抬暗部（适合中心、边长<=1 的物体）
                # 核心思想：1个强主光塑形 + 多个弱大补光抬暗部 + 轮廓光提边缘 + 顶光控制整体
                light_configs = [
                    # 1) 主光 Key：大 AREA，负责塑形（方向明确）
                    {"name": "Key_Area", "type": "AREA",
                    "position": (2.4, -2.8, 2.6), "energy": 140.0 * intensity, "size": 3.2,
                    "color": (1.0, 0.98, 0.95)},

                    # 2) 补光 Fill1：更大、更弱，从相机侧抬暗面（别太强，避免变平）
                    {"name": "Fill_Soft", "type": "AREA",
                    "position": (-2.6, -2.2, 2.0), "energy": 65.0 * intensity, "size": 4.8,
                    "color": (0.95, 0.97, 1.0)},

                    # 3) 侧向补光 Fill2：极弱，专门抬“最暗的侧面”
                    {"name": "Fill_Side", "type": "AREA",
                    "position": (0.0, 3.2, 1.5), "energy": 40.0 * intensity, "size": 5.0,
                    "color": (1.0, 1.0, 1.0)},

                    # 4) 轮廓光 Rim：从后上方打边缘，增强轮廓（能量不要太大）
                    {"name": "Rim_Light", "type": "SPOT",
                    "position": (-2.0, 3.0, 3.2), "energy": 90.0 * intensity,
                    "color": (1.0, 1.0, 1.0)},

                    # 5) 顶光 Top：很软，统一提亮，不抢主光
                    {"name": "Top_Soft", "type": "AREA",
                    "position": (0.0, 0.0, 5.5), "energy": 35.0 * intensity, "size": 5.5,
                    "color": (1.0, 1.0, 1.0)},

                    # 6) 地面“反弹”/底部微补：避免底部死黑（非常弱）
                    {"name": "Bottom_Bounce", "type": "AREA",
                    "position": (0.0, 0.0, -2.2), "energy": 20.0 * intensity, "size": 6.0,
                    "color": (1.0, 1.0, 1.0)},
                ]
            else:
                # 默认三点光
                light_configs = [
                    {"name": "Key_Light", "type": "SUN", "position": (4, -4, 6), "energy": 3.0 * intensity},
                ]
            ...
            for config in light_configs:
                ...
                # 让光源朝向原点
                import mathutils
                direction = mathutils.Vector((0, 0, 0)) - light_obj.location
                rot_quat = direction.to_track_quat('-Z', 'Y')
                light_obj.rotation_euler = rot_quat.to_euler()
                
                lights_created.append(config["name"])
            ...
            if bg_node:
                bg_node.inputs["Color"].default_value = (0.05, 0.05, 0.05, 1.0)  # 深灰色背景
                bg_node.inputs["Strength"].default_value = 0.3  # 环境光强度
            ...
            raise Exception(f"Setup lighting error: {str(e)}")

    def set_camera_position(self, position, look_at=(0, 0, 0)): 
        """
        设置相机位置和朝向
        ...
        try:
            ...
            # 计算相机朝向
            look_at_vec = mathutils.Vector(look_at)
            direction = look_at_vec - camera.location
            ...
            raise Exception(f"Set camera position error: {str(e)}")

    def render_scene(self, output_path, resolution=(1024, 1024), samples=128, engine="CYCLES"):
        """
        渲染当前场景
        ...
        try:
            ...
            # 确保有相机
            if not scene.camera:
                raise ValueError("No camera in scene. Use set_camera_position first.")
            ...
            elif engine.upper() in ["EEVEE", "BLENDER_EEVEE", "BLENDER_EEVEE_NEXT"]:
                # Blender 4.2+ 使用 BLENDER_EEVEE_NEXT
